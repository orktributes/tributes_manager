...
func _on_button_pressed():
	print("DEBUG: removeing children of bluesky GridContainer")
	img_limit_index = 0
	for n in $ScrollContainer/GridContainer.get_children():
		$ScrollContainer/GridContainer.remove_child(n)
	
	print("DEBUG: finding and adding images to bluesky GridContainer")
	for i in DirAccess.get_directories_at("user://tributes/bluesky/"):
		for ii in DirAccess.get_files_at("user://tributes/bluesky/"+i+"/src"):
			if ii == "source.txt":
				pass
			else:
				var textrect = textrect_pre.instantiate()
				textrect.dir_name = i
				textrect.sites = "bluesky"
				textrect.img_name = ii
				var image = Image.load_from_file("user://tributes/bluesky/"+i+"/src/"+ii)
				array_ran_img.append(i+"\\"+ii)
				image.resize(128,128)
				var texture = ImageTexture.create_from_image(image)
				textrect.texture = texture
				$ScrollContainer/GridContainer.add_child(textrect)
				print("path of the bluesky data DEBUG: "+"user://tributes/bluesky/"+i+"/src/"+ii)
...
func _on_button_2_pressed() -> void:
	...
	#OS.shell_open(OS.get_user_data_dir()+"/tributes/bluesky/"+file_path.get_slice("\\",0)+"/src/"+file_path.get_slice("\\",1))
	DirAccess.copy_absolute(
		"user://tributes/bluesky/"+file_path.get_slice("\\",0)+"/src/"+file_path.get_slice("\\",1),
